...
!      character*80 texte
	...
	allocate (izut(neto+1)) !allocate (izut(mmmax))
	izut(:) = 0
...
c  --------------------------------------------
      read(iu_10(iboat),*) texte            ! lecture du titre
      read(iu_10(iboat),*) ncondi
      if(ncondi.eq.0) goto 910
      do 900 i=1,ncondi
  900 read(iu_10(iboat),*) (nno(i,j),j=1,2)
  910 continue
...
c     impressions
c     ---------------
	if(langue==1) write(iu_11(iboat),210)  ! cond. de bord
	if(langue==2) write(iu_11(iboat),310)  ! cond. de bord
      do 931 i=1,ntn
        ix=nno(i,2)+1
        write(iu_11(iboat),214)(nno(i,j),j=1,2),att(ix)
  931 continue

      if(langue==1) write(iu_11(iboat),211) ! n° des noeuds
      if(langue==2) write(iu_11(iboat),311) ! n° des noeuds
      do 867 i=1,neto
        write(iu_11(iboat),216)i,(noeud(i,j),j=1,2)
  867 continue

      if(nc.ne.0) then
        if(langue==1) write(iu_11(iboat),212) ! rel. de compatibilité
        if(langue==2) write(iu_11(iboat),312) ! rel. de compatibilité
        do 876 i=1,nc
          write(iu_11(iboat),216)i,(mcomp(i,j),j=1,2)
  876   continue
      endif
...
c     --------------------------------
      ...
      if(i1.ne.i2) then
        ...
        write(*,*)  'stop'
        write(iu_11(iboat),*) 'subr. modif:'										!sept06
        write(iu_11(iboat),*)
     * 'le nombre de relation de compatibilté est errone.'	!sept06
        write(iu_11(iboat),*)
     * 'il doit y avoir une erreur dans les donnees.'		!sept06
        write(iu_11(iboat),*) 'vérifier les noh de chaque panneau.'					!sept06
        write(iu_11(iboat),*)
     * 'noh= liste des panneaux qui suivent le panneau',		!sept06
     *              ' concerne'
        
	  write(iu_14(iboat),*) 'subr. modif:'										!sept06		!bug
        write(iu_14(iboat),*)
     * 'le nombre de relation de compatibilté est errone.'	!sept06		!bug	
        write(iu_14(iboat),*)
     * 'il doit y avoir une erreur dans les donnees.'		!sept06		!bug
        write(iu_14(iboat),*)
     * 'vérifier les noh de chaque panneau.'					!sept06		!bug
        write(iu_14(iboat),*)
     * 'noh= liste des panneaux qui suivent le panneau',		!sept06		!bug
     *              ' concerne'											!sept06													!bug
