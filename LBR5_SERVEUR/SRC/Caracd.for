      subroutine caracd(e,a,b,c,d,v,f,g,h,o,p,q,const,sh,
     *  fam,mt,ksr,ksa,kst,phil,rayon,w,const2,const3,
     *  epais,itype,isect,dpo,spec)                                                                             !février 2004

      use param_section,CONST_=>CONST,CONST2_=>CONST2,B_=>B,
     *CONST3_=>CONST3,FAM_=>FAM,ITYPE_=>ITYPE,E_=>E,A_=>A,SH_=>SH,
     *ISECT_=>ISECT,SPEC_=>SPEC
...
c	  -dérivée par rapport à epais pour épontille					!février 2004
  17  if(epais.le.0.000001)  goto 18									!février 2004
	...
	write(iu_11(iboat),*) 
     *         'erreur cette section n''a pas de variable'' epais'	!février 2004		
	stop															!février 2004	
		endif														!février 2004
...
c  - dérivée par rapport à l'épaisseur (delta=c) :	
      dpbord(1)=(rayon*phil*pi/180.d00)*specc
c  - dérivée par rapport à h(âme) (d=hya) :
      dpaig(2)=((const2(1,2)*(rayon+kksa*d/2.)+ const(3)*kksa/2.)
     *			*phil*pi/180.d00) *specc/e
c  - dérivée par rapport à ep(âme) (v=dya) :
      dpaig(3)=(const2(1,3)*(rayon+kksa*d/2.)*phil*pi/180.d00)*specc/e
c  - dérivée par rapport à l(semelle) (f=wya) :
      dpaig(4)=(const2(1,4)*(rayon+kksa*d/2.)*phil*pi/180.d00)*specc/e
c  - dérivée par rapport à l'entredistance (a=epsa) :
	dpaig(5)=(const2(1,5)*(rayon+kksa*d/2.)*phil*pi/180.d00)*specc/e
c  - dérivée par rapport à h(âme) (h=hxr) :
	dpraid(6)=(const2(1,6)*rayon*phil*pi/180.d00)*specc/e
c  - dérivée par rapport à ep(âme) (o=dxr) :
	dpraid(7)=(const2(1,7)*rayon*phil*pi/180.d00)*specc/e
c  - dérivée par rapport à l(semelle) (p=wxr) :
	dpraid(8)=(const2(1,8)*rayon*phil*pi/180.d00)*specc/e
c  - dérivée par rapport à l'entredistance (b=epsr) :
	dpraid(9)=(const2(1,9)*rayon*phil*pi/180.d00)*specc/e
