      subroutine equil(mt,z2,lamb,xi,xf,ivaria,phil,ijk,q,
     *                 nel,ind,aaa,bbb,ccc,ddd,aa1,bb1,cc1,dd1,delto)

	
      use param_section, AAA_ => AAA, BBB_ => BBB, CCC_ => CCC, 
     *DELTO_ => DELTO, KSE_ => KSE, IVARIA_ => IVARIA, XF_ => XF,
     *XI_ => XI,DDD_=>DDD
...
c***********************************************************************
	...
	if (nel.eq.1) then !indispensable pour le multistructure
		som = 0.d00
		phmax = 0.d00
	endif

      s0=dsin(pi*ijk/2.)
      teta=panneau(nel).teta !tetaq(nel)
      phx=0.
      fmo=0.
      fmv=0.
...
c     ---------------- 
      ...
      p4_loc=(q**3)*6.*aa1			!rem p4_loc car p4 déjà utilisé dans sharedvar  !jan09
      fmo=(p1-p3)*s2+(p2-p4_loc)*cc2
     *              +s1*(2.*bb1*q*q-dd1)+cc1*(6.*aa1*(q**3)-cc1*q)
      fmv=(p3-p1)*cc2+(p2-p4_loc)*s2
...
c     recherche de la composante maximale
  ...
  304 write(iu_11(iboat),100) pho,fmo,phv,fmv,phx
...
c     -----------------------------------
      call pinteg(phil,teta,40,45,som,z2,mt,q,pi,panneau(nel).abtr(:),
     *            cc1,cc2,s1,s2,3,bof)
  ...
  201 som=som-2.*a1(i)*dsin((teta+panneau(nel).abtr(i))*pi/180.)

  200 som=som+phv+fmv
      write(iu_11(iboat),101)som
      if(dabs(ph).lt.0.01) goto 1
      pe=som/ph
      write(iu_11(iboat),105)pe
c
c     equilibre suivant l'axe ox
c     --------------------------
c
    1 call pinteg(phil,teta,39,0,som,z2,mt,q,pi,panneau(nel).abtr(:),
     *            cc1,cc2,s1,s2,2,vmax)
...
c     -----------------------------------
c
  2   call pinteg(phil,teta,45,40,som,z2,mt,q,pi,panneau(nel).abtr(:),
     *            cc1,cc2,s1,s2,1,bof)
     ...
     * +(2./lamb)*(z2(1276)*cc1+z2(1225)*s1-z2(1306)*cc2-z2(1255)*s2)
      ...
      do 33 i=1,mt
      ji=6*(i-1)
      sam=2.*a1(i)*dcos((teta+panneau(nel).abtr(i))*pi/180.)
   33 som=som+sam
    6 write(iu_11(iboat),103)som
