     ...
     *          disa(720),dvara(33,9,16),dtemp(33,9,38),dvarh(33,9,38)
      ...
      dimension darg(8,9)
	dimension dcha(100,9),de(9),by(1710),transp(33)  !!!aout06
...
c       8 : exp(-alpha q (2pi-(yo-y))) * sin (béta q (2pi-(yo-y)))
   ...
   4  if(dabs(teta).ne.90.) goto 5
    ...
    6 continue
      ...
      call br(arg,darg,m,q,disa,dvara,dtemp, 2,1,
     *        sint,cost,sintt,costt,nxi,nbrxi,si,co,ex)
      call br(arg,darg,m,q,disa,dvara,dtemp, 1,2,
     *        sint,cost,sintt,costt,nxi,nbrxi,si,co,ex)
      call br(arg,darg,m,q,disa,dvara,dtemp,29,3,
     *        sint,cost,sintt,costt,nxi,nbrxi,si,co,ex)
      call br(arg,darg,m,q,disa,dvara,dtemp,25,4,
     *        sint,cost,sintt,costt,nxi,nbrxi,si,co,ex)
      call br(arg,darg,m,q,disa,dvara,dtemp,27,5,
     *        sint,cost,sintt,costt,nxi,nbrxi,si,co,ex)
      call br(arg,darg,m,q,disa,dvara,dtemp,24,6,
     *        sint,cost,sintt,costt,nxi,nbrxi,si,co,ex)
      call br(arg,darg,m,q,disa,dvara,dtemp,26,7,
     *        sint,cost,sintt,costt,nxi,nbrxi,si,co,ex)
      call br(arg,darg,m,q,disa,dvara,dtemp, 3,8,
     *        sint,cost,sintt,costt,nxi,nbrxi,si,co,ex)
      call br(arg,darg,m,q,disa,dvara,dtemp,35,9,
     *        sint,cost,sintt,costt,nxi,nbrxi,si,co,ex)
      call br(arg,darg,m,q,disa,dvara,dtemp,38,10,
     *        sint,cost,sintt,costt,nxi,nbrxi,si,co,ex)
      call br(arg,darg,m,q,disa,dvara,dtemp,31,11,
     *        sint,cost,sintt,costt,nxi,nbrxi,si,co,ex)
      call br(arg,darg,m,q,disa,dvara,dtemp,30,12,
     *        sint,cost,sintt,costt,nxi,nbrxi,si,co,ex)
      call br(arg,darg,m,q,disa,dvara,dtemp,33,13,
     *        sint,cost,sintt,costt,nxi,nbrxi,si,co,ex)
      do 2 j=1,mt
      k1=2+j*2
      k2=12+j*2
      call br(arg,darg,m,q,disa,dvara,dtemp,k1,k2,
     *        sint,cost,sintt,costt,nxi,nbrxi,si,co,ex)
      k1=k1+1
      k2=k2+1
      call br(arg,darg,m,q,disa,dvara,dtemp,k1,k2,
 ...
 156  continue													   !!!aout04
   ...
   1  dtemp(ii,k,j)=0.
      ...
      subroutine br(arg,darg,m,q,disa,dvara,dtemp,i,ii,
     *              sint,cost,sintt,costt,nxi,nbrxi,si,co,ex)
c     ==============
      implicit double precision(a-h,o-z)
...
c      write(iu_11(iboat),*)'al,be,dal,dbe'
c      write(iu_11(iboat),'(6e14.7)')al,be,dal,dbe
...
c      write(iu_11(iboat),*)'u1,du1,s1,ds1,dr1'
c      write(iu_11(iboat),'(6e14.7)')u1,du1,s1,ds1,dr1
...
c      write(iu_11(iboat),*)'up,um,dup,dum,bp,bm'
c      write(iu_11(iboat),'(6e12.5)')up,um,dup,dum,bp,bm
...
c      write(iu_11(iboat),*)'usp,z1,z2,dz1,dz2'
c      write(iu_11(iboat),'(6e14.7)')usp,z1,z2,dz1,dz2
...
c      write(iu_11(iboat),*)'bpp,bmm,ap,am'
c      write(iu_11(iboat),'(6e14.7)')bpp,bmm,ap,am
c      write(iu_11(iboat),*)'dbpp,dbmm,dap,dam'
c      write(iu_11(iboat),'(6e14.7)')dbpp,dbmm,dap,dam

      cos1=ex(j)*co(j)
      sin1=ex(j)*si(j)

c      write(iu_11(iboat),*)' cos1    sin1=',cos1,sin1
c      write(iu_11(iboat),*)' cost    sint=',cost,sint
c      write(iu_11(iboat),*)' costt  sintt=',costt,sintt
...
c      write(iu_11(iboat),*)' aa,bb,daa,dbb'
c      write(iu_11(iboat),'(6e14.7)')aa,bb,daa,dbb
c      write(iu_11(iboat),*)'t3,t4'
c      write(iu_11(iboat),'(6e14.7)')t3,t4
c      write(iu_11(iboat),*)'dt3,dt4'
c      write(iu_11(iboat),'(6e14.7)')dt3,dt4

      if(ii.ge.14) goto 1
      goto(1,2,2,1,2,1,2,1,1,2,2,1,2),ii

c     termes u, ny, my ,w + wøø,vø + xo,zo  (c.à.d. 1,4,6,8 + 9,12 + 14,
c     ------------------------------------------------------------------
   1  temp=t3*cos1+t4*sin1
      dtemp(ii,k,6)=dtemp(ii,k,6) - (
     * z3*(dt3 + dal2*temp - cos1*(dt3+dbe2*t4) - sin1*(dt4-dbe2*t3) )
     *  - (t3-temp)*dnom      )/(z4*q)
c      write(iu_11(iboat),*)'temp=',temp
...
c      write(iu_11(iboat),*)'z3 et dz3=dnom'
c      write(iu_11(iboat),'(6e14.7)')z3,dnom
c      fctie=(aa*(temp3+2.*al*up)+bb*(temp4+2.*be*um))
c      write(iu_11(iboat),*)'fct et dfct'
c      write(iu_11(iboat),'(6e14.7)')fctie,temp

c      write(iu_11(iboat),*)' z4=',z4
c      write(iu_11(iboat),*)' q=',q
c      write(iu_11(iboat),*)' dtemp(ii,k,5)=',dtemp(ii,k,5)
c      write(iu_11(iboat),*)' '
