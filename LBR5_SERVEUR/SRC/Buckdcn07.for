      subroutine buckdcn07(nel,e1,sigy1,delta,hxr,dxr,wxr,txr,epsr,
     *				   epsa,q,eff,ibuck)			!juil07
...
c	  ometpart=0.
c	  do ipan=1,neto
c	    read(iu_25(iboat)) epsap,epsrp,deltap,hyap,dyap,wyap,tyap,		
...
c	--------------------------
	...
	call annuld(ddbuckpl_07,9*neto)
	...
	!	ix = 2
	!endif
	temp1 = eff(4335+1+(ix-1)*51)
	temp2 = eff(4335+31+(ix-1)*51)

c	if((sig1.ge.0.).and.(sig2.ge.0.)) then	
	if((temp1.ge.0.).and.(temp2.ge.0.)) then	!panneau en traction
	  dbuckpl_07=0.
	  goto 30											
	else
	  sig1= -temp1 !-eff(4540) !sigx au debut pan en X=L/2
	  sig2= -temp2 !-eff(4570) !sigx à la fin du pan en X=L/2
c	  sig1=-sig1
c	  sig2=-sig2
...
c		    dsig2(ll,ipan)=-dsig2(ll,ipan)			
   ...
   40	    continue !rewind iu_25(iboat)
		...
		tya2=panneau(nel).tya2
          ...
          phil=-dabs(panneau(nel).phil)*pi/180. !TODO vérifier pas abs(phil) ?
		if(epsa2.ge.(0.00001)) then
...
c	      c=1.3							!floors or deep girders (primary supporting members)
     ...
     *					       (delta/bucksp)**2+2*m*delta/bucksp) !**2)	!juil07 TODO CHECK
		    else
     ...
     *						  dpsi(ll,ipan)/(psi+1.1)**2
				  if((nel.eq.ipan).and.(ll.eq.1)) then
     	                dsigepl(ll,ipan)=0.9*e1*(dm(ll,ipan)*
     *					             (delta/dabs(q*phil))**2+
     *								 2*m*delta/dabs(q*phil)**2) !TODO CHECK
				  else
     ...
     *					             dm(ll,ipan)
		  ...
		  if(sig1.ge.sig2) then												!critère d'euler
	        ddbuckpl_07(ll,ipan)=(dsig1(ll,ipan)*sigc-
     *					      siga*dsigepl(ll,ipan))/sigc**2
	      else
	        ddbuckpl_07(ll,ipan)=(dsig2(ll,ipan)*sigc-
     *					      siga*dsigepl(ll,ipan))/sigc**2
		  endif
	    enddo
	  enddo
    ...
    2 format(i3,15x,f10.3,16x,f10.3,3x,f6.3,6x,'plate')

   30 if(ibuck.eq.1) then 
		deallocate (dpsi)
		deallocate (dm)
		deallocate (dsigepl)
...
c	---------------------------------
	...
	call annuld(ddbuckstif_07,9*neto)
	
	if(dbuckpl_07.eq.0.) then	
...
c	          dsig1(ll,ipan)=-sens2(10,1,ll,ipan)
	          ddbuckstif_07(ll,ipan)=(dsig1(ll,ipan)*sigc-
     *						     siga*dsigestif(ll,ipan)/beta)/sigc**2
	        else
c	          dsig2(ll,ipan)=-sens2(10,3,ll,ipan)
			  ddbuckstif_07(ll,ipan)=(dsig2(ll,ipan)*sigc-
     *						     siga*dsigestif(ll,ipan)/beta)/sigc**2
			endif
	      else
			if(sig1.ge.sig2) then
c			  dsig1(ll,ipan)=-sens2(10,1,ll,ipan)
     ...
     *							  beta)/sigc**2
	 ...
	 dsigc(ll) = - 0.25*sigy1*(-
     * sigy1*dsigestif(ll,ipan))/(sigestif**2*beta)

	 test = (dsig1(ll,ipan)*sigc - siga*dsigc(ll))/sigc**2

	 test = (dsig1(ll,ipan)*sigc - 0.25*siga*sigy1*(
     * -sigy1*dsigestif(ll,ipan))/(sigestif**2*beta) )/sigc**2
			else
c	          dsig2(ll,ipan)=-sens2(10,3,ll,ipan)
			  ddbuckstif_07(ll,ipan)=(dsig2(ll,ipan)*sigc-
     *							  siga*sigy1*(0.25*sigy1*
     ...
     *							  beta)/sigc**2
	  ...
	  if(itera.ge.1) then
	  write(iu_31(iboat),4) nel,1.e-6*sigestif,1.e-6*sigc,dbuckstif_07
	  endif
    ...
    4 format(i3,4x,f10.3,3x,f10.3,3x,f8.5,6x,'stiffener')      
