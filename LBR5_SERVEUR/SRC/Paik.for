      subroutine paik(nel,e1,sigy1,delta,epsa,hxr,dxr,wxr,txr,epsr,entr,
     *                dspaik,spaik,omt)
...
c     -----------------------------------------------------------------------
	...
	if(slen.gt.slim) then
	  write(iu_31(iboat),*)'attention: slen limite dépassé =',slim,
     *                  ' << slen(réel) =',slen
	  write(iu_31(iboat),*)'=========='
	  write(iu_14(iboat),*)'attention: slen limite dépassé =',slim,					!sept06		!bug
...
c     dans ce cas le calcul des sensibilités se fait par différences finies.
      ...
      kk=nxit(k,nel,iboat)
        ...
        if(kk.eq.1) then
           ratio=1.05
         xxx=ratio *delta
         call ulk(e1,sigy1,xxx,epsa,hxr,dxr,wxr,txr,entr,spaik1)
           dspaik(kk)=(spaik1-spaik)/(xxx-delta)
        else if (kk.eq.5) then                      ! xi=epsa
	   ratio=0.5
         xxx=ratio *epsa
         call ulk(e1,sigy1,delta,xxx,hxr,dxr,wxr,txr,entr,spaik1)
   ...
   16    ind=ind+1
	   ratio=5.0*ind
         xxx=ratio *hxr
         call ulk(e1,sigy1,delta,epsa,xxx,dxr,wxr,txr,entr,
     *            spaik1)
           ...
           ratio=10.0
         xxx=ratio *dxr
         call ulk(e1,sigy1,delta,epsa,hxr,xxx,wxr,txr,entr,
     *            spaik1)
           dspaik(kk)=(spaik1-spaik)/(xxx-dxr)
        else if (kk.eq.8) then                       ! xi= d flange
	   ratio=10.0
         xxx=ratio *wxr
         call ulk(e1,sigy1,delta,epsa,hxr,dxr,xxx,txr,entr,
     *            spaik1)
	   dspaik(kk)=(spaik1-spaik)/(xxx-wxr)
        else if (kk.eq.9) then                  ! xi=epsr
	   ratio=0.95
         xxx=ratio *entr
         call ulk(e1,sigy1,delta,epsa,hxr,dxr,wxr,txr,xxx,spaik1)
...
c ******************************************************************************
      ...
      subroutine ulk(e1,sigy1,delta,epsa,hxr,dxr,wxr,txr,entr,
     *               spaik)
