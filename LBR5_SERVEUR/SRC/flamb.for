	...
	use param_section,SIGY_=>SIGY,E_=>E
...
c	 compression pure
	 ...
	 write(iu_11(iboat),31) res1,res2,sol   

	  if (res1.le.sol) then								
	  write(iu_11(iboat),32) 
	  write(iu_14(iboat),32)		!sept06
	 else
	  write(iu_11(iboat),33) 
	  endif
	  if (res2.le.sol) then								
	  write(iu_11(iboat),34)
	  write(iu_14(iboat),34)		!sept06
	 else
	  write(iu_11(iboat),35) 
	  endif				
	 else  ! vmymax .gt. 100
c	 flexion composée 
...
c	exc2= 1/(1-(f/(sur*euler2)))
	  ...
	  sol2=sols			!+++
	  res=sig/(1.02*1.15) 
		write(iu_11(iboat),38) sol1,sol2,res		!+++
...
c	 flambement en flexion composée selon le cticm (eurocode3)
     ...
     *  (elanb2.lt.0.2).and.(cond2.lt.0.1)) then		!biappuyé
		write(iu_11(iboat),*) 'bizarre, flamb biencastré et pas biappuyé'
	 	 write(iu_11(iboat),36) sbrd1,sbrd2,asy		!en n/mm²
