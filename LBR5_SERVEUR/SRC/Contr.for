     ...
     *                 sigplaque,dsigplaque,ratio)     !dad
      ...
      use param_section, temp_sxm => sxm !pour éviter conflit
...
c
c     calcul des nbrxi dérivées des restrictions dc(i,j), i=1,nbrxi [nbrxi=nvar(nel,iboat)]
...
c -----------------------------------
	...
	m=m1tabl1(nel)  ! nbre de restrictions du cas de charge is pour le panneau nel
	  ...
	  write(iu_31(iboat),'(a,i3)') 'panneau nø ', nel
	  write(iu_31(iboat),'(a)')    '-----------------'
        write(iu_31(iboat),284) m,(ypts(i,nel),i=1,panneau(nel).ipts)   ! ypts (ordonnées des pts)
      endif
...
c       contrainte sbord(iy,ix);    iy=iy2  et ix=5 pour x=l/2
	...
	! Test si existence de raid !!! Sinon pas adéquat d'utiliser cette restriction
...
c       contrainte sajas(iy,ix);    iy=iy2  et ix=1 pour x=0
	...
	! Test si existence de semelle !!! Sinon pas adéquat d'utiliser cette restriction
...
! Test si existence de cadres !!! Sinon pas adéquat d'utiliser cette restriction
...
c       contrainte srjab(iy,ix);    iy=iy2  et ix=1 pour x=0
	...
	! Test si existence de raidisseurs !!! Sinon pas adéquat d'utiliser cette restriction
	if (panneau(nel).hxr.le.0.001) then
		write(*,*) 'Attention ! Panneau :',nel
...
c     restrictions nø 101: sigmaX dans le bordé en z=0 (x=l/2)
c     ----------------------------------------------------------------
        vnom2_='b.SX z=0;l/2'
...
c     restrictions nø 102: sigmaY dans le bordé en z=0 (x=l/2)
c     ----------------------------------------------------------------
        vnom2_='b.SY z=0;l/2'
...
c     restrictions nø 103: sigmaX (+STIFF) dans le bordé en z=0 (x=l/2)
c     ----------------------------------------------------------------
	  vnom2_='b.SX + Stiff'
...
c     restrictions nø 104: sigmaY cadre jas à mi-portée (x=l/2)
c     ----------------------------------------------------------------
        vnom2_='c.JAS SY;l/2'
...
c     restrictions nø 105: TauYZ cadre jas à mi-portée (x=l/2)
c     ----------------------------------------------------------------
        vnom2_='c.JAS T; l/2'
...
c     restrictions nø 106: sigmaY cadre jab à mi-portée (x=l/2)
c     ----------------------------------------------------------------
        vnom2_='c.JAB SY;l/2'
...
c     restrictions nø 107: TauYZ cadre jab à mi-portée (x=l/2)
c     ----------------------------------------------------------------
        vnom2_='c.JAB T; l/2'
...
c     restrictions nø 108: sigmaY cadre semelle à mi-portée (x=l/2)
c     ----------------------------------------------------------------
        vnom2_='c.SEM SY;l/2'
...
c     restrictions nø 109: TauYZ cadre semelle à mi-portée (x=l/2)
c     ----------------------------------------------------------------
        vnom2_='c.JSEM T; l/2'
...
c     -----------------------------------------------------------
	  ...
	  write(iu_14(iboat),*)
     *' la restriction nø',ic,' n''existe pas !!!'					!bug
        write(iu_14(iboat),*)' dans panneau n°',nel,' !!!'									!bug
        write(iu_14(iboat),*)' veuillez svp corriger les données.'							!bug
...
c     fin de la boucle principale sur le nbre de restrictions
  ...
  10  write(*,*) 'dans subr. contr : it=0 . impossible -> error'
     ...
     * 'positions de calcul des restrictions relatives'
      write(iu_31(iboat),*)
     * 'aux traverses sont erronés (subr. contr)'
     ...
     * 'positions de calcul des restrictions relatives'					!bug
      write(iu_14(iboat),*)
     * 'aux traverses sont erronés (subr. contr)'							!bug
...
c     ----------------------------------------------------------------- 
  ...
  427 format('c(',i4,') > ',e15.3,' de type nø',i4,' (',a12,')',
     *       ' au pt ypt nø',i2,' et au pt de calcul nø',i2)
  428 format('c(',i4,') < ',e15.3,' de type nø',i4,' (',a12,')',
     *       ' au pt ypt nø',i2,' et au pt de calcul nø',i2)
      end
