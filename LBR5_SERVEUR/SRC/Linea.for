	...
	use param_section, COEFF_=>COEFF,DCOEFF_=>DCOEFF,DARG_=>DARG,Z_=>Z,
     *SINU_=>SINU,COSI_=>COSI,EXPT_=>EXPT,IMPR_=>IMPR,IOPTI_=>IOPTI
 
      implicit double precision(a-h,o-z)
      dimension coeff(32,41),sx(360),ip(4),x(720),k(4),
     *          dcoeff(32,33,9),dpx(8,9),px(8),darg(8,9),nxi(9),
     *          j(4),jk(4)
...
c***********************************************************************
  ...
  111 x(i)=0.d00
     ...
     *       cosi(k(4))+coeff(ip(3),j(i))*sinu(k(4)))
      ...
      write(iu_11(iboat),'(i3,4e12.5)') 
     *i,aux(i),aux(20+i),aux(40+i),aux(60+i)
   11 continue

      do 13 i=1,400
   13 aux2(i)=aux(i)

      call sysmat(aux,z,4,1,sx,20)
      
      if(impr.eq.0) goto 112
      write(iu_11(iboat),*) 'terme indép.'
      write(iu_11(iboat),'(4e12.5)') (sx(ik),ik=1,4)
      write(iu_11(iboat),*)  ' '
  112 continue
...
c     corps de la matrice (idem cas normal)
   ...
   44   continue
   45   continue
        if(impr.ne.0) write(iu_11(iboat),47) kl,(dpx(i,kl),i=1,8)
   46 continue
