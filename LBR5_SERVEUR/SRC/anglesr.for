      subroutine anglesr(nel,phil,aux,zb)
...
c**********************************************************************
      ns =nsign(nel)
      an =panneau(nel).angle*pi/180.d00
      an2=an-phil*pi/180.d00

      if(dabs(panneau(nel).angle).ne.180.d00) goto 4
      c=-1.
      s=0.
      c29=-dcos(-phil*pi/180.d00)
      s2=-dsin(-phil*pi/180.d00)
      goto 6

   4  if(dabs(panneau(nel).angle).ne.90.) goto 5
      ii=1
      if(panneau(nel).angle.eq.-90.) ii=-1
      c=0.
      s=ii
      c29=-dsin(-phil*pi/180.d00)*ii
      s2=+dcos(-phil*pi/180.d00)*ii
      goto 6
