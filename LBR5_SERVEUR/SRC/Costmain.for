      subroutine costmain(totalrro,vlength,numv,derivee,
...
c     
c ce module est appelé si icout = 2 (sinon c'est le module objct1 qui est utilisé, calcul simplifié)
c
c modifications:
...
c
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c	   open (unit=iu_36(iboat), file='dbinput.txt',          status='old',err=906) !juillet2003
c	   open (unit=iu_37(iboat), file='dbfractionnement.txt', status='old')
c	   open (unit=iu_38(iboat), file='dbacces.txt',          status='old',err=908) !juillet2003
c        open (unit=iu_39(iboat), file='dbbase.txt',           status='old',err=904) !juillet2003
c 	   open (unit=iu_42(iboat), file='dbsoudures.txt',       status='old',err=905) !juillet2003
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc

	implicit double precision (a-g,k,o-z)
      dimension trav(277565)			!sept06
	dimension derivee(nder)
   
      open (unit=iu_39(iboat), file='dbbase.txt',     
     *status='old',err=904) !juillet2003
	open (unit=iu_42(iboat), file='dbsoudures.txt', 
     *status='old',err=905) !juillet2003

	read(iu_39(iboat),*) ! on ne lit pas la première ligne qui est vide
	read(  iu_39(iboat),*) nnn  ! = nombre d'opérations
	rewind iu_39(iboat)
	close( iu_39(iboat))
      noid = 25      ! = nombre id soudure (il faudrait pouvoir le lire directement
	               !                      dans le fichier base.txt)
      read(iu_42(iboat),*)
	read(iu_42(iboat),*) nropid ! = nombre d'opérations qui nécessitent id soudure  (max=8)
	rewind iu_42(iboat)
	close(iu_42(iboat))
...
c       = 275525   
     ...
     *  deltacsc,deltar,vhar,vlarg,vepaiss,vhac,veac,
     *  vlength,neto,noncost,
     *  numv,nnn,noid,nropid,j1,j2,derivee,
     *  nder,nvar(:,iboat),nxit(:,:,iboat),totalti,
     *  coutpan)
        
	return
  904 write(*,*) 'stop : the "dbbase.txt" file is missing'     !juillet2003
      write(iu_14(iboat),*)'stop : the "dbbase.txt" file is missing'	   !sept06	!bug
	read(*,*)
	stop                                                   !juillet2003
  905 write(*,*) 'stop : the "dbsoudures.txt" file is missing' !juillet2003
      write(iu_14(iboat),*)'stop : the "dbsoudures.txt" file is missing' !sept06	!bug
	read(*,*)
	stop                                                   !juillet2003
	end
...
c =================================================================================
	...
	use param_section, ONLY : panneau,iu_14,iu_36,iu_40,iu_37,iu_38,iu_42,
     *iu_39,iboat

	implicit double precision (a-g,k,o-z)
	integer kst,kkjj
	double precision memosou,memovart,memovars,memotol
	
	!TODO : allouer dynamiquement ce qui suit !!
c	dimension iop(nnn),ivtolier(nnn),ivsoudeur(nnn),tolier(nnn,100),	!tolier(nnn,neto) janv2007
c     *   soudeur(nnn,100), variablet(nnn,maxd), variables(nnn,maxd),		!soudeur(nnn,neto) janv2007
c     *   c(j1,j2),iv(nnn,irowmax),irow(nnn),
c     *   nci(neto),nanp(neto),idpanneau(neto),ippanneau(neto),
c     *   itpanneau(neto),nttapes(neto),npabouts(neto),k(nnn,neto),
c     *   ca(nnn,neto),cat(nnn,neto),div(2),variables1(nnn,maxd),
c     *   soudeur1(nnn,maxd),variablet1(nnn,maxd),tolier1(nnn,maxd),
c     *   iid(noid),irid(noid),is(noid,iridmax),ts(noid,maxs),
c     *   vst(noid,maxs),ss(noid,maxs),vss(noid,maxs),ivts(noid),
c     *   ivss(noid),idsoudures(nropid,neto),vgorsoud(nropid,neto),
c ----------------
c     *   vlarg(neto),vepaiss(neto),vhac(neto),veac(neto),
c     *   deltacsc(neto),vhar(neto),
c     *   deltar(neto),
c     *   q(nnn,neto),
c ----------------
c     *   cut(nnn,neto),cus(nnn,neto),rrotol(nnn,neto),rrosoud(nnn,neto),
c     *   titol(nnn,neto),tisoud(nnn,neto),
c     *   memovart(maxd),memovars(maxd),memosou(maxd),
c     *   memotol(maxd),
c ----------------
c     *   pantolti(neto),pantolrro(neto),pansoudti(neto),
c     *   pansoudrro(neto),pantotalti(neto),pantotalrro(neto),
c ----------------
c     *   t12(nnn,neto),t15(nnn,neto),t16(nnn,neto),t19(nnn,neto),
c     *   t21s(nnn,neto),t23s(nnn,neto),t26s(nnn,neto),
c     *   t21t(nnn,neto),t23t(nnn,neto),t26t(nnn,neto),
c -----------------
c     *   derivee(nder),nvar(neto),nxit(9,neto)


      dimension iop(100),ivtolier(100),ivsoudeur(100),tolier(100,100),	!tolier(nnn,neto) janv2007
     *   soudeur(100,100), variablet(100,35), variables(100,35),		!soudeur(nnn,neto) janv2007
     *   c(j1,j2),iv(100,10),irow(100),
     *   nci(200),nanp(200),idpanneau(200),ippanneau(200),
     *   itpanneau(200),nttapes(200),npabouts(200),k(100,200),
     *   ca(100,200),cat(100,200),div(2),variables1(100,35),
     *   soudeur1(100,35),variablet1(100,35),tolier1(100,35),
     *   iid(25),irid(25),is(25,3),ts(25,21),
     *   vst(25,21),ss(25,21),vss(25,21),ivts(25),
     *   ivss(25),idsoudures(8,200),vgorsoud(8,200),
c ----------------
     *   vlarg(200),vepaiss(200),vhac(200),veac(200),
     *   deltacsc(200),vhar(200),
     *   deltar(200),
     *   q(100,200),
c ----------------
     *   cut(100,200),cus(100,200),rrotol(100,200),rrosoud(100,200),
     *   titol(100,200),tisoud(100,200),
     *   memovart(35),memovars(35),memosou(35),
     *   memotol(35),
c ----------------
     *   pantolti(200),pantolrro(200),pansoudti(200),
     *   pansoudrro(200),pantotalti(200),pantotalrro(200),
c ----------------
     *   t12(100,200),t15(100,200),t16(100,neto),t19(100,200),
     *   t21s(100,200),t23s(100,200),t26s(100,200),
     *   t21t(100,200),t23t(100,200),t26t(100,200),
c -----------------
     *   derivee(nder),nvar(200),nxit(9,200)

ccc	dimension tolier2(52,35),soudeur2(52,35),num(35) 

	
	! Common ci-dessous plus nécessaire a priori		!jan09

c      common/cout/ poids,spoids,
c     *        fmat,fsou,fmdo,fmat11,fmat22,fmat33,fsou11,fsou22,
c     *        fsou33,fmdo11,fmdo22,fmdo33,fmdo44,fmdo55,fmdo66,fmdo77,
c     *      rend,eqp,dref,drefx,drefy,c11,c22,c33,dc11,dw2,dw3,p10,dp10,
c     *        p4,p5,dp4,dp5,p9x,p9y,dp9x,dp9y,ber,bet,p6,p7,c8,dc8,cout,
c     *        ialr,ialt      !nécessaire pour avoir icout
c	common/cout/icout
...
c 1.0	open files			
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
	open (unit=iu_36(iboat), file='dbinput.txt', status='old',err=906) 
	open (unit=iu_39(iboat), file='dbbase.txt' , status='old')
	open (unit=iu_40(iboat), file='resultats cout.txt' 
     *, status='unknown')
!	open (unit=iu_43(iboat), file='sensibilites.txt'      
!     *, status='unknown')
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
	write(iu_40(iboat),*) 'fichier des resultats'
	write(iu_40(iboat),*) '====================='
	write(iu_40(iboat),*) ' '
c
	write(iu_40(iboat),*) 'verification input module cout'
	write(iu_40(iboat),*) '------------------------------'
	write(iu_40(iboat),*) ' '
	read(iu_36(iboat),*) net !Ne sert plus à rien en fait
	if (net.ne.neto) then
		write(*,*) 'Attention, la 1ere ligne du fichier dbInput.txt co
     *rrespond au nbr de panneau dans la structure. Ce nbr n''est pas co
     *rrect'
		write(*,*) 'Veuillez corriger vos donnees - appuyer sur ENTER 
     *pour poursuivre'
		read(*,*)
		stop
	endif
	read(iu_36(iboat),*) nfract
	read(iu_36(iboat),*) ncacces
	read(iu_36(iboat),*) ncatelier
	write(iu_40(iboat),*) 'nombre panneaux'
	write(iu_40(iboat),112) neto
	write(iu_40(iboat),*) ' '

	if (nfract.eq.0) then
	  write(iu_40(iboat),*) 'fractionnement par defaut'
	  write(iu_40(iboat),*) ' '
	else
	  write(iu_40(iboat),*) 'fractionnement defini par l utilisateur'
	  write(iu_40(iboat),*) ' '
	endif

	if (ncacces.eq.0) then
	  write(iu_40(iboat),*) 'coeff acces par defaut'
	  write(iu_40(iboat),*) ' '
	else
	  write(iu_40(iboat),*) 'coeff acces defini par l utilisateur'
	  write(iu_40(iboat),*) ' '
	endif

	if (ncatelier.eq.0) then
	  write(iu_40(iboat),*) 'coeff atelier par defaut'
	  write(iu_40(iboat),*) ' '
	else
	  write(iu_40(iboat),*) 'coeff atelier defini par l utilisateur'
	  write(iu_40(iboat),*) ' '
	endif

	read(iu_36(iboat),*) nam
	read(iu_36(iboat),*) pmb
	read(iu_36(iboat),*) (div(i),i=1,2)
	write(iu_40(iboat),*) ' '
	write(iu_40(iboat),*) 'nombre abouts montage','    ',nam
	write(iu_40(iboat),*) 'cout preparer-mettre-balancer','  ',pmb
	write(iu_40(iboat),*) 'cout divers tolier','  ',div(1)
	write(iu_40(iboat),*) 'cout divers soudeur','  ',div(2)
	write(iu_40(iboat),*) ' '	
		
		read(iu_36(iboat),*) (nci(i),i=1,neto)
		read(iu_36(iboat),*) (nanp(i),i=1,neto)
		read(iu_36(iboat),*) (idpanneau(i),i=1,neto)
		read(iu_36(iboat),*) (ippanneau(i),i=1,neto)
		read(iu_36(iboat),*) (itpanneau(i),i=1,neto)
		read(iu_36(iboat),*) (nttapes(i),i=1,neto)
		read(iu_36(iboat),*) (npabouts(i),i=1,neto)

	write(iu_40(iboat),*) 'no','   ','nci','   ','nanp','   ','id panneau',
     *'   ','ip panneau','   ','it panneau','   ','type tapes','   ',
     *'p abouts'

	do i=1,neto
	  write(iu_40(iboat),*) i,nci(i),nanp(i),idpanneau(i)
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
	write(iu_40(iboat),*) 
     *'verification de la lecture de la base de donnees'
	write(iu_40(iboat),*) 
     *'------------------------------------------------'
 
	read(iu_39(iboat),*) ! on ne lit pas la 1ère ligne, qui est vide
	read(iu_39(iboat),*) nnn
	write(iu_40(iboat),*) 'nombre operations = ',nnn
	write(iu_40(iboat),*) ' '
...
c     -----------------
      do 778 i=1,nnn  
	  read(iu_39(iboat),*) iop(i),irow(i)
c	  write(iu_40(iboat),*) ' operation',' ',iop(i)
c	  write(iu_40(iboat),*) ' '
	  do j=1,irow(i)
	    read(iu_39(iboat),*) iv(i,j),(c(j,ijk),ijk=1,iv(i,j))
	  enddo
...
122	do 777 j=1,irow(i)  
	...
	if(iv(i,j).eq.1.and.j.eq.1) then 
	  tolier(i,1)=c(j,1)
c	  write(iu_40(iboat),*) 'cout unitaire tolier =',tolier(i,1)
	  ivtolier(i)=1
	endif

	if(iv(i,j).eq.1.and.j.ne.1) then
	  soudeur(i,1)=c(j,1)
c	  write(iu_40(iboat),*) 'cout unitaire soudeur =',soudeur(i,1)
	  ivsoudeur(i)=1
	endif
	...
	if(iv(i,1).ne.1) then 
	  ...
	  enddo
c	  write(iu_40(iboat),*) 'variable',(variablet(i,ijk),ijk=1,iv(i,j))
c	  write(iu_40(iboat),*) 'tolier  ',(tolier(i,ijk),ijk=1,iv(i,j))
c	  write(iu_40(iboat),*) ' '
   	  ivsoudeur(i)=iv(i,3)
	endif
...
cccccccccccccccccccccccccccccccccc
	if(i.eq.19.or.i.eq.20) then
	  ...
	  enddo
c	  write(iu_40(iboat),*) 'cout unitaire soudeur'
c	  write(iu_40(iboat),*) soudeur(i,1),'  si epaiss <',variables(i,1)
c	  write(iu_40(iboat),*) soudeur(i,2),'  si epaiss >',variables(i,2)
c	  write(iu_40(iboat),*) ' '
	  ivsoudeur(i)=iv(i,2)
	endif
...
ccccccccccccccccccccccccccccccccccc
	if(i.eq.25.or.i.eq.26) then
	  ...
	  enddo
c	  write(iu_40(iboat),*) 'hauteur             ',(variablet(i,ijk),ijk=1,iv(i,j))
c	  write(iu_40(iboat),*) 'largeur             ',(variablet1(i,ijk),
c      *                                  ijk=1,iv(i,j))
c	  write(iu_40(iboat),*) 'tolier recouvrement ',(tolier(i,ijk),ijk=1,iv(i,j))
c	  write(iu_40(iboat),*) 'soudeur recouvrement',(soudeur(i,ijk),ijk=1,iv(i,j))
c	  write(iu_40(iboat),*) 'tolier encastrees   ',(tolier1(i,ijk),ijk=1,iv(i,j))
c	  write(iu_40(iboat),*) 'soudeur encastrees  ',(soudeur1(i,ijk),ijk=1,iv(i,j))
c	  write(iu_40(iboat),*) ' '
	  ivtolier(i)=iv(i,j)
	  ivsoudeur(i)=iv(i,j)
	endif
...
cccccccccccccccccccccccccccccccccccc	
	if(i.eq.30.or.i.eq.31) then
	  ...
	  enddo
c	write(iu_40(iboat),*) 'hauteur ',(variablet(i,ijk),ijk=1,iv(i,j))
c	write(iu_40(iboat),*) 'largeur ',(variablet1(i,ijk),ijk=1,iv(i,j))
c	write(iu_40(iboat),*) 'tolier  ',(tolier(i,ijk),ijk=1,iv(i,j))
c	write(iu_40(iboat),*) 'soudeur ',(soudeur(i,ijk),ijk=1,iv(i,j))
c	write(iu_40(iboat),*) ' '
	  ivtolier(i)=iv(i,j)
	  ivsoudeur(i)=iv(i,j)
...
c	enddo
c	write(iu_40(iboat),*) 'variable',(variables(i,ijk),ijk=1,iv(i,j))
c	write(iu_40(iboat),*) 'soudeur ',(soudeur(i,ijk),ijk=1,iv(i,j))
c	write(iu_40(iboat),*) ' '
c	endif
c ----------------------------
...
c ==================================================
	...
	do i=1,nnn
	  write(iu_40(iboat),*) ' operation',' ',iop(i)
	  write(iu_40(iboat),*) ' '
	  write(iu_40(iboat),*) ' '
	  write(iu_40(iboat),*) 'variable tolier'
	  write(iu_40(iboat),*) (variablet(i,j),j=1,ivtolier(i))
	  write(iu_40(iboat),*) ' '
	  write(iu_40(iboat),*) 'cout tolier'
	  write(iu_40(iboat),*) (tolier(i,j),j=1,ivtolier(i))
	  write(iu_40(iboat),*) ' '
	  write(iu_40(iboat),*) 'variable soudeur'
	  write(iu_40(iboat),*) (variables(i,j),j=1,ivsoudeur(i))
	  write(iu_40(iboat),*) ' '
	  write(iu_40(iboat),*) 'cout soudeur'
	  write(iu_40(iboat),*) (soudeur(i,j),j=1,ivsoudeur(i))
	  write(iu_40(iboat),*) ' '
	enddo
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
	read(iu_39(iboat),*) ctspp
	write (iu_40(iboat),*) '%%%%%%%%%%%%%%%%%%%%%%'
	write (iu_40(iboat),*) 'coeff pre-pre sur soudeur', ctspp
	read(iu_39(iboat),*) cttpp
	write (iu_40(iboat),*) 'coeff pre-pre sur tolier', cttpp
	read(iu_39(iboat),*) cbn
	write (iu_40(iboat),*) 'coeff balayage nappe', cbn
	read(iu_39(iboat),*) cmn
	write (iu_40(iboat),*) 'coeff minorateur nappe', cmn
	read(iu_39(iboat),*) cbp
	write (iu_40(iboat),*) 'coeff balayage pre', cbp
	read(iu_39(iboat),*) crp
	write (iu_40(iboat),*) 'coeff reformage pre', crp

	write (iu_40(iboat),*) '%%%%%%%%%%%%%%%%%%%%%%'	
	write (iu_40(iboat),*) ' '
	write (iu_40(iboat),*) 'id soudures'
	write (iu_40(iboat),*) '==========='
	write (iu_40(iboat),*) ' '
	read(iu_39(iboat),*) noid
	write(iu_40(iboat),*) 'nombre id soudures =',noid
	write (iu_40(iboat),*) ' '
c
	do 780 i=1,noid  ! 780

	read(iu_39(iboat),*) iid(i),irid(i)
c	write(iu_40(iboat),*) ' id',' ',iid(i)
c	write(iu_40(iboat),*) ' '

	do j=1,irid(i)
	  read(iu_39(iboat),*) is(i,j),(c(j,ijk),ijk=1,is(i,j))
	enddo
c
	do j=1,irid(i) 
	  if(is(i,j).eq.1.and.j.eq.1) then 
  	    ts(i,1)=c(j,1)
c	    write(iu_40(iboat),*) 'cout unitaire tolier =',ts(i,1)
	    ivts(i)=1
	  endif
	  if(is(i,j).eq.1.and.j.ne.1) then
	    ss(i,1)=c(j,1)
c	    write(iu_40(iboat),*) 'cout unitaire soudeur =',ss(i,1)
	    ivss(i)=1
	  endif
	  ...
	  if(is(i,j).ne.1.and.j.eq.1) then 
	    do ijk=1,is(i,j)
	      ...
	      ts(i,ijk)=c(j+1,ijk)
	    enddo
c	    write(iu_40(iboat),*) 'variable',(vst(i,ijk),ijk=1,is(i,j))
c	    write(iu_40(iboat),*) 'tolier  ',(ts(i,ijk),ijk=1,is(i,j))
c	    write(iu_40(iboat),*) ' '
	    ivts(i)=is(i,j)
	  endif
...
c	do i=1,noid
c	write(iu_40(iboat),*) ' id soudure',' ',iid(i)
c	write(iu_40(iboat),*) ' '
c	write(iu_40(iboat),*) ' '
c	write(iu_40(iboat),*) 'variable tolier'
c	write(iu_40(iboat),*) (vst(i,j),j=1,ivts(i))
c	write(iu_40(iboat),*) ' '
c	write(iu_40(iboat),*) 'cout tolier'
c	write(iu_40(iboat),*) (ts(i,j),j=1,ivts(i))
c	write(iu_40(iboat),*) ' '
c	write(iu_40(iboat),*) 'variable soudeur'
c	write(iu_40(iboat),*) (vss(i,j),j=1,ivss(i))
c	write(iu_40(iboat),*) ' '
c	write(iu_40(iboat),*) 'cout soudeur'
c	write(iu_40(iboat),*) (ss(i,j),j=1,ivss(i))
c	write(iu_40(iboat),*) ' '
c	enddo
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
	if (nfract.eq.1) then
	  ...
	  open (unit=iu_37(iboat),
     *file='dbfractionnement.txt',status='old',err=907) !juillet2003

	  read(iu_37(iboat),*) nnnnn
c	  write(*,*) 'nnnnn',nnnnn
	  read(iu_37(iboat),*) nkkkk
c	  write(*,*) nkkkk

	  if(nnnnn.ne.neto) then
	    write(*,*)'nbr de panneaux incompatible: fractionnement-input'
	    write(iu_14(iboat),*)
     *'nbr de panneaux incompatible: fractionnement-input'		!sept06	!bug
	    read(*,*)
		stop
	  endif

	  read(iu_37(iboat),*) ((k(i,j),j=1,nnnnn),i=1,nkkkk)
...
c ---------------------------------------------------
	write(iu_40(iboat),*) ' '
	write(iu_40(iboat),*) 'verification fractionnement'
	write(iu_40(iboat),*) '==========================='

	do j=1,neto
	 write(iu_40(iboat),*) 'panneau ', j
	 write(iu_40(iboat),*) '============'
	 write(iu_40(iboat),*) ' '
	 do i=1,nnn
	   write(iu_40(iboat),*) 'operation',i,'   ',k(i,j)
	 enddo
	 write(iu_40(iboat),*) '%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%'
	 write(iu_40(iboat),*) ' '
	enddo
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
	if (nfract.eq.1.and.ncatelier.eq.1) then
	  ...
	  open(unit=iu_38(iboat),file='dbacces.txt', status='old',err=908) !juillet2003
	  ...
	  if(nn.ne.neto) then
		...
		write(iu_14(iboat),*)'subroutine costmain'								!sept06			!bug
		write(iu_14(iboat),*)'nombre des panneaux incompatible acces-i
     *nput'		!sept06			!bug
		read(*,*)
	    stop
	  endif
     ...
     *               '-fractionnement'
		write(iu_14(iboat),*)' '													!sept06			!bug
		write(iu_14(iboat),*)'subroutine costmain'								!sept06			!bug
		write(iu_14(iboat),*)'nombre des opérations incompatible acces
     *',			!sept06
     *			   '-fractionnement'									!sept06
	  endif

	  read(iu_38(iboat),*) ((ca(i,j) ,j=1,nn),i=1,nk)
	  read(iu_38(iboat),*) ((cat(i,j),j=1,nn),i=1,nk)
...
c ---------------------------------------------------
	write(iu_40(iboat),*) ' '
	write(iu_40(iboat),*) 'verification acces et atelier'
	write(iu_40(iboat),*) '==========================='
	
	do i=1,neto
	  write(iu_40(iboat),*) 'panneau ',i
	  write(iu_40(iboat),*) '==========='
	  write(iu_40(iboat),*) ' '
	  do j=1,nnn
	 write(iu_40(iboat),*) 'operation',j,'   ',ca(j,i),'    ',cat(j,i)
	  enddo
	  write(iu_40(iboat),*) '%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%'
	  write(iu_40(iboat),*) ' '
	enddo 
...
c     entr
      ...
      write(iu_40(iboat),*)
     *'verification de la lecture des parametres des panneaux'
	write(iu_40(iboat),*) 
     *'-----------------------------------------------------'
      write(iu_40(iboat),*) 'longueur des panneaux =',vlength
	write(iu_40(iboat),*) ' '

	do i=1,neto
		vlarg(i) = -(panneau(i)%hight)
	enddo

c imprimer sur le fichier "8" chaque paramètre de chaque panneau ??????

!!!avril2003  do i=1,neto
!!!avril2003  read(iu_41(iboat),*) nopan  ,vlarg(i),vepaiss(i)
!!!avril2003	read(iu_41(iboat),*) vhac(i),veac(i),vlsc(i),vesc(i),deltac(i)
!!!avril2003	read(iu_41(iboat),*) vhar(i),vear(i),vlsr(i),vesr(i),deltar(i),entr(i)
cc	vv1(i)=vepaiss(i)
cc	vv2(i)=vhac(i)
cc	vv3(i)=veac(i)
cc	vv4(i)=vlsc(i)
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
	...
	open (unit=iu_42(iboat),file='dbsoudures.txt', status='old')

	write(iu_40(iboat),*) ' '
	write(iu_40(iboat),*) 'verification id soudures'
	write(iu_40(iboat),*) '========================'

	read(iu_42(iboat),*) nrpanneaux
	read(iu_42(iboat),*) nropid

	if(nnnnn.ne.nrpanneaux) then
	  write(*,*) 'le nombre de panneaux est incompatible:',
     *  'fractionnement-soudures'
	  write(iu_14(iboat),*)' '																!bug
	  write(iu_14(iboat),*)'subroutine costmain'												!bug
	  write(iu_14(iboat),*) 'le nombre de panneaux est incompatible:',						!bug
     *  'fractionnement-soudures'	
		read(*,*)
		stop
	endif
	...
	if(neto.ne.nrpanneaux) then
	  ...
	  write(iu_14(iboat),*)'subroutine costmain'												!bug
	 write(iu_14(iboat),*)
     *'le nombre de panneaux est incompatible:soudure-input'				!bug
	  read(*,*)
	  stop
	endif

c	write(iu_40(iboat),*) 
c	write(iu_40(iboat),*) 'nombre panneaux =',nrpanneaux
c	write(iu_40(iboat),*) 'nombre operations qui necessitent id',nropid

	read(iu_42(iboat),*) ((idsoudures(i,j),j=1,nrpanneaux),i=1,nropid)		
	read(iu_42(iboat),*) ((vgorsoud(i,j),j=1,nrpanneaux),i=1,nropid)
	
c	do j=1,nrpanneaux
c	  write(iu_40(iboat),*) ' '
c	  write(iu_40(iboat),*) 'panneau',j
c	  write(iu_40(iboat),*) ' '
c	  write(iu_40(iboat),*) 'id soudures'
c	  write(iu_40(iboat),*) 'op 21',idsoudures(1,j)
c	  write(iu_40(iboat),*) 'op 22',idsoudures(2,j)
c	  write(iu_40(iboat),*) 'op 23',idsoudures(3,j)
c	  write(iu_40(iboat),*) 'op 36',idsoudures(4,j)
c	  write(iu_40(iboat),*) 'op 37',idsoudures(5,j)
c	  write(iu_40(iboat),*) 'op 47',idsoudures(6,j)
c	  write(iu_40(iboat),*) 'op 48',idsoudures(7,j)
c	  write(iu_40(iboat),*) 'op 51',idsoudures(8,j)
c	  write(iu_40(iboat),*) ' '
c	  write(iu_40(iboat),*) 'gorges soudures'
c	  write(iu_40(iboat),*) 'op 21',vgorsoud(1,j)
c	  write(iu_40(iboat),*) 'op 22',vgorsoud(2,j)
c	  write(iu_40(iboat),*) 'op 23',vgorsoud(3,j)
c	  write(iu_40(iboat),*) 'op 36',vgorsoud(4,j)
c	  write(iu_40(iboat),*) 'op 37',vgorsoud(5,j)
c	  write(iu_40(iboat),*) 'op 47',vgorsoud(6,j)
c	  write(iu_40(iboat),*) 'op 48',vgorsoud(7,j)
c	  write(iu_40(iboat),*) 'op 51',vgorsoud(8,j)
c	enddo
...
c     write(iu_40(iboat),*) 'calcul des couts unitaires (interpolation lineaire)'
c	write(iu_40(iboat),*) '---------------------------------------------------'
...
c         extrapolation ???????????                            ! donnee + pas
          cus(j,i) = soudeur(j,1)
	    if (id.eq.numv.and.k(j,i).gt.0) noncost = 100 + j  ! dans ce cas, message d'alerte !          
...
c         extrapolation ???????????????
	    ...
	    if (id.eq.numv.and.k(j,i).gt.0) noncost = 200 + j
...
c         extrapolation ???????????
          cut(j,i) = tolier(j,1)
	    if (id.eq.numv.and.k(j,i).gt.0) noncost = 100 + j
...
!	close(iu_43(iboat))
  ...
  906 write(*,*) 'stop : the "input.txt" file is missing'           !juillet2003
	write(iu_14(iboat),*) 'stop : the "input.txt" file is missing'							!bug
  907 write(*,*) 'stop : the "fractionnement.txt" file is missing'  !juillet2003
	write(iu_14(iboat),*) 'stop : the "fractionnement.txt" file is missing'					!bug
  908 write(*,*) 'stop : the "acces.txt" file is missing'           !juillet2003
      write(iu_14(iboat),*) 'stop : the "acces.txt" file is missing'							!bug
	stop                                                          !juillet2003
	end
...
c	implicit double precision (a-g,o-z)
