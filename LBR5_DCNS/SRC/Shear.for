	subroutine shear(thicktau,yneuttau,dyneuttau,itau,ditau,
     *				 tau,dtau)
...
c*********************************************************************************
	...
	call annuld(wq,2*neto)
	call annuld(tmean,neto)
	call annuld(list,ntn)
	call annuld(fact,neto)
	call annuld(dwq,2*neto*neto)
...
c	---------------------------------------------------------------------------------------------------------------------------
		  ...
		  if(list(i).eq.noeud(nel,1)) then
		    ...
		    do j=1,np
			  if(noeud(nel,2).eq.list(j)) then
			    aux(i,j)=-part(nel)*thicktau(nel)/hight
	          endif
			enddo
    1			aux(i,i)=aux(i,i)+part(nel)*thicktau(nel)/hight
		    zb(i)=zb(i)+0.5*part(nel)*thicktau(nel)*hight*
     *			  (y1+fact/3.*hight*sin(teta))			
		  elseif(list(i).eq.noeud(nel,2)) then
			...
			enddo
    2			aux(i,i)=aux(i,i)+part(nel)*thicktau(nel)/hight
		    zb(i)=zb(i)+0.5*part(nel)*thicktau(nel)*hight*
     *		      (y1+fact/3.*hight*sin(teta))
		  endif
	    endif
...
c	----------------------------------------------------------------------------------------------------
     ...
     *		    then
		        ...
		        do j=1,np
			      if(noeud(nel,2).eq.list(j)) then
			        aux(i,j)=-part(nel)*(dthicktau*hight-thicktau(nel)*
     *					     dhight)/hight**2
	              endif
			    enddo
    3			    aux(i,i)=aux(i,i)+part(nel)*(dthicktau*hight-
     *			         thicktau(nel)*dhight)/hight**2
			    zb(i)=zb(i)+0.5*part(nel)*(dthicktau*hight*
     ...
     *					     dhight)/hight**2
			    ...
			    enddo
    4			    aux(i,i)=aux(i,i)+part(nel)*(dthicktau*hight-
     *			         thicktau(nel)*dhight)/hight**2
			    zb(i)=zb(i)+0.5*part(nel)*(dthicktau*hight*
     *			      (y1+fact/3.*hight*sin(teta))+thicktau(nel)*
     *			      (dhight*(y1+fact/3.*hight*sin(teta))+hight*
