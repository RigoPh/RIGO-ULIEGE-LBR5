     ...
     *                 q,jjk,indaig,indrai,nel,iprint,
...
c     resist. ult selon la form. empirique de paik (subr ulk incluse dans subr paik)
      if(nz==jjk) then
        call ulk(iprint,e1,sigy1,delta,epsa,hxr,dxr,wxr,txr,entr,spaik) !! entr et pas epsr
	  spk(nel)=spaik
	endif
...
c  --------------------------------------------------------------------------
	...
	stt7 =sp *e1*(tm*(z2(ip+1632)+z2(ip+1581)/q) - tpb*z2(ip+1887))/dya	! tyz jas (non cumule)
c     stt7 = temp(ij) =tyz jas  mais non cumule
	...
	stt8=sp *e1*(tmr*(z2(ip+1632)+z2(ip+1581)/q) - tpr*z2(ip+1887))/dya	!tyz jab (non cumule)
      if((k.eq.3).and.(nel.eq.ipan)) then
